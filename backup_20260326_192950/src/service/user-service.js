...
const userService = {
    ...
    async resetPassword(c, params, userId) {
        ...
        if (password.length < 6) throw new BizError(t('pwdMinLength'));
        const { salt, hash } = await cryptoUtils.hashPassword(password);
        await orm(c).update(user).set({ password: hash, salt: salt }).where(eq(user.userId, userId)).run();
    },

    selectByEmail(c, email) {
        return orm(c).select().from(user).where(and(eq(user.email, email), eq(user.isDel, isDel.NORMAL))).get();
    },
    ...
    selectById(c, userId) {
        return orm(c).select().from(user).where(and(eq(user.userId, userId), eq(user.isDel, isDel.NORMAL))).get();
    },
    ...
    async list(c, params) {
        ...
        const query = orm(c).select({ ...user, username: oauth.username, trustLevel: oauth.trustLevel, avatar: oauth.avatar, name: oauth.name })
        ...
        const { total } = await orm(c).select({ total: count() }).from(user).where(and(...conditions)).get();
        ...
        for (const u of list) {
            ...
            u.receiveEmailCount = receiveMap[userId] || 0;
            u.sendEmailCount = sendMap[userId] || 0;
            u.accountCount = accountMap[userId] || 0;
            u.delReceiveEmailCount = delReceiveMap[userId] || 0;
            u.delSendEmailCount = delSendMap[userId] || 0;
            u.delAccountCount = delAccountMap[userId] || 0;

            const roleIndex = roleList.findIndex(r => u.type === r.roleId);
            let sendAction = {};
            if (roleIndex > -1) {
    ...
    async updateUserInfo(c, userId, recordCreateIp = false) {
        ...
        const params = { os, browser, device, activeIp, activeTime: dayjs().format('YYYY-MM-DD HH:mm:ss') };
        if (recordCreateIp) params.createIp = activeIp;
    ...
    async incrUserSendCount(c, quantity, userId) {
        await orm(c).update(user).set({ sendCount: sql`${user.sendCount} + ${quantity}` }).where(eq(user.userId, userId)).run();
    },
    ...
    async add(c, params) {
        ...
        if (!c.env.domain.includes(emailUtils.getDomain(email))) throw new BizError(t('notEmailDomain'));
        if (password.length < 6) throw new BizError(t('pwdMinLength'));
    ...
    listByRegKeyId(c, regKeyId) {
        return orm(c).select({ email: user.email, createTime: user.createTime }).from(user).where(eq(user.regKeyId, regKeyId)).orderBy(desc(user.userId)).all();
    }
};
