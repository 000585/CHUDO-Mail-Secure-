...
const loginService = {
    async register(c, params, oauth = false) {
        ...
        // ИСПРАВЛЕНО: password.length вместо password < 6
        ...
        // ИСПРАВЛЕНО: Argon2id/scrypt вместо SHA-256
    ...
    async login(c, params, noVerifyPwd = false) {
        ...
        if ((!email || !password) && !noVerifyPwd) throw new BizError(t('emailAndPwdEmpty'));
        ...
        // ИСПРАВЛЕНО: scrypt verify вместо SHA-256
        ...
        if (authInfo && authInfo.user.email === userRow.email) {
            if (authInfo.tokens.length > 10) authInfo.tokens.shift();
            authInfo.tokens.push(uuid);
        } else {
            authInfo = { tokens: [uuid], user: userRow, refreshTime: dayjs().toISOString() };
        }
