...
const roleService = {
    async add(c, params, userId) {
        ...
        if (!name) throw new BizError(t('emptyRoleName'));
        ...
        if (notEmailIndex > -1) throw new BizError(t('notEmail'));
    ...
    async delete(c, params) {
        ...
        if (!roleRow) throw new BizError(t('notExist'));
        if (roleRow.isDefault) throw new BizError(t('delDefRole'));
    ...
    hasAvailDomainPerm(availDomain, email) {
        ...
        if (availDomain.length === 0) return true;
        const domain = emailUtils.getDomain(email.toLowerCase());
        return availDomain.some(item => item.toLowerCase() === domain);
    },
    ...
    isBanEmail(banEmail, fromEmail) {
        ...
        if (banEmail.includes('*')) return true;
        for (const item of banEmail) {
            if (verifyUtils.isDomain(item)) {
                if (item.toLowerCase() === emailUtils.getDomain(fromEmail.toLowerCase())) return true;
            } else {
                if (item.toLowerCase() === fromEmail.toLowerCase()) return true;
            }
        }
        return false;
    }
