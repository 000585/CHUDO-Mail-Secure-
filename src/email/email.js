...
export async function email(message, env, ctx) {
    try {
                // PATCH: Лимит размера входящего email (25MB)
        ...
        while (true) {
            ...
            if (totalSize > MAX_EMAIL_SIZE) {
                message.setReject('Message too large (max 25MB)');
                return;
            }
            chunks.push(value);
        }
        
        const content = new TextDecoder().decode(Buffer.concat ? Buffer.concat(chunks) : chunks);
        const {
            receive,
            tgChatId,
            tgBotStatus,
